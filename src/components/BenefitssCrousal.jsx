...
import "../styles/benefitsCrousal.css";

// import required modules
import { EffectCoverflow, Navigation, Pagination } from "swiper/modules";

const BenefitssCrousal = ({ path, slides }) => {
  const [active, setActive] = useState(0);

  return (
    <div className="lg:flex justify-between w-full items-center p-16  hidden relative">
      <div className="swiper-button-prev slider-arrow  border border-solid rounded-full p-2">
      ...
      >
        {slides.map((ele) => {
          return (
            <SwiperSlide>
              <img src={ele.img_src} alt="logo" />
              <p className="text-3xl text-[#0D283A] font-bold">{ele.title}</p>
              <p className="text-xl text-[#1D3C51]">{ele.description}</p>
            </SwiperSlide>
          );
        })}
      </Swiper>

      <div className="swiper-button-next slider-arrow  border border-solid rounded-full p-2">
      ...
      <div className='absolute bottom-[10px] right-[60px] flex flex-col items-center -rotate-45'>
